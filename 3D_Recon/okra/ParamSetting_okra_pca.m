%{ 
Parameter setting corresponding to Imaging geometry used at the time
of acquiring measurements.
%}


function ParamSetting_okra_pca(angStepSize,outDirectory)

param.nx = 338; %number of voxels
param.ny = 338;
param.nz = 123;

param.sx = 107.6;% mm (real size)
param.sy = 107.6; % mm 
param.sz = 39; % mm  
...
param.su = 336*0.15*8;	% mm (real size)
param.sv = 156*0.15*8;     % mm

% X-ray source and detector setting
param.DSD = 785.5;    %  Distance source to detector 
param.DSO = 210;	  %  X-ray source to object axis distance

% angle setting
param.dir = 1;            % gantry rotating direction (clock wise/ counter clockwise)
param.dang = angStepSize; % angular step size (deg)
param.deg =  0:param.dang:359.5; 
...
filename = sprintf('%s/parameters_okra_pca.mat', outDirectory);
save(filename,'param');
