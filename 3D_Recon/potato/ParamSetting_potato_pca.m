...
function ParamSetting_potato_pca(angStepSize,outDirectory)
...
param.nu = 150;	% number of pixels
param.nv = 150;

% Detector setting (real size)
param.su = 150*0.15*4;	% mm (real size) % subsampling fector of 4
param.sv = 150*0.15*4;     % mm

% X-ray source and detector setting
param.DSD = 261.8;    %  Distance source to detector 
param.DSO = 70;	%  X-ray source to object axis distance

% angle setting
param.dir = 1;   % gantry rotating direction (clock wise/ counter clockwise)
param.dang = angStepSize; % angular step size (deg)
param.deg =  0:param.dang:359.9; % you can change
