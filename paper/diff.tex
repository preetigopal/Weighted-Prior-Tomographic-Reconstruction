...
%DIF DEL /tmp/old.tex   Fri Jul 10 19:25:21 2020
%DIF ADD tci.tex        Fri Jul 10 19:24:09 2020
%% bare_jrnl.tex
%% V1.4b
%% 2015/08/26
%% by Michael Shell
...
$\boldsymbol{\theta}$ of the object $\boldsymbol{x}$. If \DIFaddbegin \DIFadd{$\boldsymbol{\Psi}$
represents the sparsity basis, i.e., if $\boldsymbol{x} = \boldsymbol{\Psi\theta}$, and 
}\DIFaddend $\boldsymbol{\mathcal{R}}$ represents the acquisition model, then the
LASSO solution is described by one that minimizes
 \begin{equation}
J_{\text{CS}}(\boldsymbol{\theta}) = \lVert\DIFdelbegin %DIFDELCMD < \boldsymbol{\mathcal{R} x}%%%
\DIFdelend \DIFaddbegin \boldsymbol{\mathcal{R} \Psi\theta}\DIFaddend - \boldsymbol{y}\rVert_2^2  + \lambda_1\lVert\boldsymbol{\theta}\rVert_1
\label{Eq:simple_cs}
 \end{equation}
...
\centering
 ...
 Section~\ref{sec:results_spatially_varying_prior}. Section\DIFaddbegin \DIFadd{~}\DIFaddend \ref{sec:discussion}
...
\label{sec:related}
\DIFdelbegin \DIFdel{Few view projection has been explored in~\mbox{%DIFAUXCMD
...
\DIFdel{add here!!  Why is this bad and how does it compare with
ours? }%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{The idea of using a reduced number of views is most pronounced in
specialized applications. For example, in~\mbox{%DIFAUXCMD
\cite{Essam2015}}%DIFAUXCMD
,
...
in~\mbox{%DIFAUXCMD
\cite{Li2015,Polak2017} }%DIFAUXCMD
can be applied.  In a more generalized
...
%DIF >   As can be seen
%DIF >  in the situatiowe will
%DIF >  later see in Figs.\ref{fig:object-prior_test_okra}
%DIF >  and~\ref{fig:object-prior_test_sprouts}, the spatial-gradient of many
%DIF >  datasets is not sparse.
\DIFaddend 
...
a set of 3D CT scans \DIFaddbegin \DIFadd{are taken }\DIFaddend over short intervals of time\DIFdelbegin \DIFdel{are taken as }\DIFdelend \DIFaddbegin \DIFadd{, and }\DIFaddend the
object undergoes changes between successive scans.  \DIFdelbegin \DIFdel{In both these
cases, it may be possible to use }\DIFdelend \DIFaddbegin \DIFadd{The reconstruction
is then performed over the entire set generating several 3D
volumes. ~\mbox{%DIFAUXCMD
\cite{PICCS} }%DIFAUXCMD
is one of the earliest work to establish the
...
at hand. This limitation was later dealt with by applying }\DIFaddend very
...
volumes is assumed to be continuous, thus enabling the use of optical
...
longitudinal studies. \DIFdelbegin \DIFdel{Fig.~\ref{fig:prior_overview} provides an
overview. }\DIFdelend In contrast to the object prior-based studies mentioned
...
in terms of time intervals \DIFdelbegin \DIFdel{such as in 4D CT
scans }\DIFdelend \DIFaddbegin \DIFadd{-- prior scans could be months apart}\DIFaddend . We
...
\DIFaddbegin \DIFadd{Fig.~\ref{fig:prior_overview} provides an overview.
}

 \DIFaddend %Various forms of object priors have been used for reconstruction from limited views (`few-views'). In ~\cite{Van2014}, the time-varying region within an object is computed by modelling it as a B-spline.
...
%DIF > We emphasize that in
 ...
 the full volumes is proprietary. }\emph{\DIFadd{Departing from this, we
   demonstrate reconstruction results from raw projection data.}}
\DIFaddend In all figures in this section, `uniform prior' refers to optimizing Eq.~\ref{eq:spatially-varying_prior} with $\boldsymbol{W}(x,y,z)=1$.
%----------------------------------------------------------
%-------------------------------------------------
...
$336\times156$. The corresponding size of the reconstructed volume is
$338\times338\times123$. Prior to the first scan, two holes were
...
truth consists of FDK reconstructed volumes from the the full set of
450 view projections. The test volume was reconstructed from a partial
set of 45 projections, i.e. $10\%$ of the projection views from which
ground truth was reconstructed. The selected 3D ground truth of
template volumes, the test volume as well as the 3D reconstructions
can be seen in~\cite{supp_paper}. One of the slices of the
reconstructed volumes is shown in Fig.~\ref{fig:okra_3D_results}. The
red and green 3D RoI in the video and images show the regions where
new changes are present. Based on the potato and the okra experiments,
...
Although the uniform global prior can be very useful in some
...
the signal, as seen in Fig.~\ref{fig:RFA2_few_views}-d. Ideally, we
...
& 0.84 & 0.86 & \textcolor{red}{0.88} \\ \hline \textbf{green RoI} & 1
(ideal) & 0.80 & \textcolor{red}{0.86} & 0.80 & \textcolor{red}{0.86}
\\ \hline
\end{tabular}
...
shown in~\cite{supp_paper}. One of the slices of the reconstructed
...
\textbf{\begin{tabular}[c]{@{}c@{}}Okra\\size:(338,338,123)\\45 views\end{tabular}} & 6 sec. & 8 min.  & 25 min. & \DIFdelbeginFL \DIFdelFL{27 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{60 }\DIFaddendFL min. \\ \hline
