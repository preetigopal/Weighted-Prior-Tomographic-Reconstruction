...
%DIF DEL post_tci.tex                                           Mon Feb  7 17:09:54 2022
%DIF ADD /home/sharat/Dropbox/students/TCI/paper/post_tci.tex   Sun Feb  6 14:04:35 2022
%% V1.4b
%% 2015/08/26
%% by Michael Shell
...
\begin{abstract}
  ...
  with these techniques is the %DIF < strong 
  \DIFaddbegin \DIFadd{strong }\DIFaddend influence of object-priors in
  ...
  for imaging in longitudinal studies. We show that with
  as little as $2\%$ of measurements, our method achieves a structural
  fidelity of $0.84$, with $1.00$ representing ideal reconstruction.
\end{abstract}
...
qualitative and quantitative results. \DIFdelbegin \DIFdel{Before we do this, it is import
...
\DIFdel{In order to show the efficacy, we will clearly need to have either a
...
top left figure in Fig.~\ref{fig:story}).  For quantitative results}\DIFdelend \DIFaddbegin \DIFadd{For the latter}\DIFaddend ,
...
absolute error. \DIFdelbegin \DIFdel{Our results depend on the optimization library (as
mentioned earlier) \mbox{%DIFAUXCMD
\cite{TVReg-lib}}\hspace{0pt}%DIFAUXCMD
...
for }\DIFdelend \DIFaddbegin \DIFadd{For }\DIFaddend purposes of normalization\DIFdelbegin \DIFdel{and a fair comparison}\DIFdelend , while reporting numbers
\DIFdelbegin \DIFdel{, we therefore }\DIFdelend \DIFaddbegin \DIFadd{we }\DIFaddend rescale intensity values of both the ground truth and the
reconstructed results to lie between 0 to 1, so that the best possible
RMSE is 0, and the worst value is 1.  \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend On the other hand, SSIM seeks to
consider perceptual changes, as well as correlation between adjacent
pixels. This can be useful in a clinical context. The SSIM~\cite{multiscale_ssim} \DIFdelbegin \DIFdel{was designed to flexible }\DIFdelend \DIFaddbegin \DIFadd{also provides a flexibility to }\DIFaddend set coefficients for the following three factors: Structure, Contrast and Intensity. \DIFdelbegin \DIFdel{We observed that in many cases, our qualitative results(e.g., when the standard TV regularization method
outperforms FDK) do not correspond favourably to the default SSIM settings. We
therefore assigned }\DIFdelend \DIFaddbegin \DIFadd{In all our results, we report SSIM with both the standard setting, i.e., with coefficients $1.0,1.0,1.0$ and a structure-focussed setting, wherein we assign a higher weightage to structure preservation. In the latter case, we assign }\DIFaddend the following coefficients to the factors Structure, Contrast, and Intensity respectively: $0.7,0.2,0.1$. Our results are optimized for structure-focussed SSIM, but
it can be verified that in all cases for the various datasets
considered, our results are favourable with either \DIFdelbegin \DIFdel{the default metric, i.
e, with
...
preservation.
}\DIFdelend \DIFaddbegin \DIFadd{metric.
}\DIFaddend 

\DIFaddbegin \DIFadd{Here below, we first discuss tuning of parameters followed by the
rationale for using multiple eigenspaces to compute weightsmap. We
then present reconstruction results for each of the datasets.
}\DIFaddend %The reason for emphasizing the structure with an
...
\DIFdelbegin \DIFdel{Finally, for a fair comparison in order to not distort metrics,
...
%DIFDELCMD < %%%
%DIF <  \subsection{Tuning of parameters}
\DIFdelend \DIFaddbegin \subsection{\DIFadd{Tuning of parameters}}
\DIFaddend \label{sec:tuning_parameters}
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Next, we turn to the parameters of the algorithm.  $\lambda_1$,
$\lambda_2$ and $k$ are the three hyper-parameters that need to be
chosen carefully for best results. Since the }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend aim of our experiments is to demonstrate the benefit of using weighted prior reconstruction \DIFaddbegin \DIFadd{when optimal hyperparameter values for $\lambda_1$}\DIFaddend , \DIFdelbegin \DIFdel{we
proceed to adjust these parameters by (as mentioned earlier) assuming
}\DIFdelend \DIFaddbegin \DIFadd{$\lambda_2$ and $k$ are chosen.  In this regard, we assume }\DIFaddend the ground-truth to be known. \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend Specifically, $\lambda_1$ was tuned to maximize the
SSIM of the complete reconstructed test volume using the TV-only
regularizer.  \DIFdelbegin \DIFdel{It's important to note that we retain this value of $\lambda_1$ for the }\DIFdelend \DIFaddbegin \DIFadd{In particular, regions of interest (RoI) are 
not known in advance, and thus not considered for the purpose of this
tuning. This optimal $\lambda_1$ was retained for }\DIFaddend spatially-varying prior-based \DIFdelbegin \DIFdel{reconstruction, i.e., we }\emph{\DIFdel{do
  not}} %DIFAUXCMD
\DIFdel{adjust this
value for our spatially-varying method, and,
neither do we know the target ROI where the metric is going to be
finally employed}\DIFdelend \DIFaddbegin \DIFadd{reconstruction}\DIFaddend . With the already chosen optimal $\lambda_1$, we apply our method for a range of $\lambda_2$ values and select the one that gives the best SSIM with reference to the ground truth. %The value of $\lambda_2$ influences the amount of artefacts that can be removed by the use of object-prior at the expense of its dominance in new regions. %This value was found to lie between $(0,3)$ for our datasets.
%DIF < In principle, one could have jointly optimized both $\lambda_1$ and
%DIF < $\lambda_2$ to get even better results compared to the one we have in
%DIF < Sections~\ref{sec:results_with_imaging_model},
%DIF < and~\ref{sec:results_without_imaging_model}.  
\DIFdelbegin \DIFdel{We note that in practice , }\DIFdelend %DIF > In principle, one could have jointly optimized both $\lambda_1$ and $\lambda_2$ to get even better results compared to the one we have in Sections~\ref{sec:results_with_imaging_model}, and~\ref{sec:results_without_imaging_model}. 
\DIFaddbegin \DIFadd{In practice }\DIFaddend however, since ground-truth will never be available, these parameters must be tuned using only the set of templates available.  One of the templates can be assumed to be a pseudo-test, and a grid search can be performed to get the best possible hyperparameter values. These values can then be used for reconstructing the `real' test volume.

%DIF < in the nvone may select $\lambda_2$ omnisciently only while imaging a
%DIF < particular class of object (say `sprouts') for the first time, and
%DIF < thereafter retain the same $\lambda_2$ while imaging other objects of
%DIF < the same class in future.  
%DIF > in the nvone may select $\lambda_2$ omnisciently only while imaging a particular class of object (say `sprouts') for the first time, and thereafter retain the same $\lambda_2$ while imaging other objects of the same class in future. 
...
\begin{table}[!h]
  \centering
\caption{Standard-SSIM (`SSIM-1'), structure-focussed SSIM (`SSIM-2') and RMSE of the reconstructions shown in
...
\begin{table}[!h]
  \centering
  \caption{Standard-SSIM, structure-focussed SSIM (RMSE) within the RoI of reconstructed potato from various
    methods. 
...
\begin{table}[!h]
  \centering
  \caption{Standard-SSIM, structure-focussed SSIM (RMSE) within the RoI of reconstructed okra from various
    methods. 
...
\begin{table}[!h]
  \centering
  \caption{Standard-SSIM, structure-focussed SSIM (RMSE) within the RoI of 2D reconstruction of radio-frequency ablation data of liver from various methods.  
...
\begin{table}[!h]
  \centering
  \caption{Standard-SSIM, structure-focussed SSIM (RMSE) within the RoI of reconstructed sprouts from various
    methods. 
%The SSIM of ideal reconstruction) is 1. The ideal RMSE is 0, and the worst RMSE possible is 1 here. 
