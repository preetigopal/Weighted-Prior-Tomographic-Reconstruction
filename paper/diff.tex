...
%DIF DEL /tmp/old.tex   Thu Jul  9 16:07:56 2020
%DIF ADD tci.tex        Thu Jul  9 18:51:56 2020
%% bare_jrnl.tex
%% V1.4b
%% 2015/08/26
%% by Michael Shell
...
\begin{abstract}
  ...
  (\DIFdelbegin \DIFdel{forming the }\DIFdelend `object-prior'), helps in the reconstruction \DIFdelbegin \DIFdel{of the current
  }\DIFdelend \DIFaddbegin \DIFadd{from the current
  measurements of that }\DIFaddend object (`test')\DIFaddbegin \DIFadd{, }\DIFaddend while requiring significantly
  ...
  angles (\DIFdelbegin \DIFdel{referred to as }\DIFdelend `views'\DIFdelbegin \DIFdel{henceforth}\DIFdelend ), it is challenging when the number of views is
  limited. 
\DIFaddbegin 

  \DIFaddend If the goal is to track the changes while simultaneously reducing
  sub-sampling artefacts, we propose (1) acquiring measurements from
  \DIFdelbegin \DIFdel{a }\DIFdelend \DIFaddbegin \DIFadd{an }\DIFaddend \emph{\DIFaddbegin \DIFadd{extremely }\DIFaddend small} number of views and using a global
  \emph{`uniform'} \DIFdelbegin \DIFdel{prior based }\DIFdelend \DIFaddbegin \DIFadd{prior-based }\DIFaddend reconstruction. If the goal is to
  observe \DIFdelbegin \emph{\DIFdel{details}} %DIFAUXCMD
\DIFdelend \DIFaddbegin \textbf{\DIFadd{details}} \DIFaddend of new changes, we propose (2) acquiring
  measurements from a \emph{moderate} number of views \DIFaddbegin \DIFadd{(albeit, still
  sub-Nyquist), }\DIFaddend and using a more involved reconstruction routine. We
  show that in the latter case, a `spatially-varying' technique is
  \DIFdelbegin \DIFdel{necessary }\DIFdelend \DIFaddbegin \DIFadd{appropriate }\DIFaddend in order to prevent the prior from adversely affecting the
...
X-rays at different orientations (`views')\DIFdelbegin \DIFdel{through the object}\DIFdelend . It is preferable to
minimize the radiation exposed \DIFdelbegin \DIFdel{to the object, }\DIFdelend in order to prevent any potential
damage to it and in order to \DIFdelbegin \DIFdel{fasten }\DIFdelend \DIFaddbegin \DIFadd{reduce }\DIFaddend the acquisition time. Therefore,
...
object $\boldsymbol{x}$ uniformly \DIFaddbegin \DIFadd{with a substantial number of views,
ideally }\DIFaddend above the Nyquist rate. In such a case when there are
sufficient measurements, reconstruction using the conventional
filtered backprojection (FBP) suffices, as seen in
Fig.~\ref{fig:story} (\DIFaddbegin \DIFadd{first column, }\DIFaddend 600 views\DIFdelbegin \DIFdel{case). }\DIFdelend \DIFaddbegin \DIFadd{). The figure shows the
...
\DIFaddend However, in the last decade, reconstruction from \DIFdelbegin \DIFdel{limited sampled data }\DIFdelend \DIFaddbegin \DIFadd{reduced views }\DIFaddend has
...
\DIFaddbegin \DIFadd{wavelet transforms, or the }\DIFaddend Discrete Cosine Transform (DCT)\DIFdelbegin \DIFdel{or wavelet transforms}\DIFdelend . This is
known as sparsity prior and is the fundamental principle in the widely
used Compressed Sensing (CS) technique~\cite{Donoho,introCS}.
\DIFdelbegin \DIFdel{Fig.~\ref{fig:story} demonstrates the benefit of using sparsity prior when the number of projection views is limited (100 views in Fig.~\ref{fig:story}).
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend There are multiple ways to incorporate the sparsity prior using CS. We use the LASSO (least absolute shrinkage and selection operator) which iteratively solves for the solution by penalizing a combination of least squares error and \DIFdelbegin \DIFdel{l1-norm }\DIFdelend \DIFaddbegin \DIFadd{$L_1$-norm }\DIFaddend of the sparse coefficients $\boldsymbol{\theta}$ of the object $\boldsymbol{x}$. If $\boldsymbol{\mathcal{R}}$ represents the acquisition model, then the LASSO solution is described by one that minimizes
 \DIFdelbegin \DIFdel{:
}%DIFDELCMD < 

%DIFDELCMD <  %%%
\DIFdelend \begin{equation}
J\DIFdelbegin \DIFdel{_{CS}}\DIFdelend \DIFaddbegin \DIFadd{_{\text{CS}}}\DIFaddend (\boldsymbol{\theta}) = \lVert\boldsymbol{\mathcal{R} x}- \boldsymbol{y}\rVert_2^2  + \lambda_1\lVert\boldsymbol{\theta}\rVert_1
\DIFdelbegin \DIFdel{.
}\DIFdelend \label{Eq:simple_cs}
 \end{equation}
 \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD <  %%%
\DIFdelend \DIFaddbegin \DIFadd{Fig.~\ref{fig:story} (second column) demonstrates the benefit of
 using sparsity prior when the number of projection views is limited
 (100 views).  }\DIFaddend We solve this cost function using the popular
 l1-regularized least squares (l1-ls) package available
 in~\cite{l1ls}\DIFdelbegin \DIFdel{. We also }\DIFdelend \DIFaddbegin \DIFadd{, and  }\DIFaddend choose DCT as our sparsity basis
 $\boldsymbol{\Upsilon}$. 
\DIFdelbegin \DIFdel{We observed that there was no significant benefit in choosing wavelets over DCT.
 }%DIFDELCMD < \begin{figure}[h]
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL 

 %DIF > %% Postpone later We observed that there was no significant benefit
 %DIF > %% in choosing wavelets over DCT.  }

 \begin{figure}[t]
\DIFaddendFL \centering
	\includegraphics[width=0.4\textwidth]{../images/story/story.png}
        \caption{Illustration of the use of various priors as the
          number of views reduces. For \DIFaddbeginFL \DIFaddFL{extremely }\DIFaddendFL large number of views
          (600 views), the FBP reconstruction \DIFaddbeginFL \DIFaddFL{(first column) }\DIFaddendFL is of
          \DIFaddbeginFL \DIFaddFL{very }\DIFaddendFL good quality (\DIFaddbeginFL \DIFaddFL{Structural Similarity Index Metric,
          }\DIFaddendFL SSIM=0.92\DIFaddbeginFL \DIFaddFL{)}\DIFaddendFL . As the number of views become limited (100
          views), the reconstruction using sparsity prior (\DIFdelbeginFL \DIFdelFL{SSIM = 0.90}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{middle
          column}\DIFaddendFL ) is better \DIFaddbeginFL \DIFaddFL{both visually and quantitatively }\DIFaddendFL than FBP
          \DIFdelbeginFL \DIFdelFL{reconstruction }\DIFdelendFL (SSIM \DIFdelbeginFL \DIFdelFL{= }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{of 0.90 vs }\DIFaddendFL 0.83). If the number of views is
          \DIFdelbeginFL \DIFdelFL{further }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{drastically }\DIFaddendFL reduced (30 views), the presence of both
          sparsity prior and object \DIFaddbeginFL \DIFaddFL{prior }\DIFaddendFL improves the reconstruction
          (SSIM = 0.85) when compared to presence of only sparsity
          prior (SSIM = 0.82) or no prior (SSIM = 0.69). \DIFaddbeginFL \DIFaddFL{In all cases,
          the ground truth (260x260) appears in the top left.}\DIFaddendFL }
 \label{fig:story}
 \end{figure} 
\DIFaddbegin 

 \DIFaddend When the number of views is significantly \DIFdelbegin \DIFdel{less}\DIFdelend \DIFaddbegin \DIFadd{reduced}\DIFaddend , the sparsity prior
 alone is not sufficient\DIFdelbegin \DIFdel{, as seen in Fig.~\ref{fig:story} for the case of 30 views}\DIFdelend .  In such cases \DIFdelbegin \DIFdel{, any additional information
 , specifc }\DIFdelend \DIFaddbegin \DIFadd{additional information
 specific }\DIFaddend to the object being scanned (i.e., object prior) is useful
 in further improving the reconstruction.  \DIFdelbegin \DIFdel{Such techniques may also use information from previously scanned data }\DIFdelend \DIFaddbegin \DIFadd{While this has been done in
 the context of dynamic CT-scans (as discussed in
 Section~\ref{sec:related}), in this paper we use a set $S$ of
 previous scans }\DIFaddend of the same object \DIFdelbegin \DIFdel{or a
 similar one.
}\DIFdelend \DIFaddbegin \DIFadd{that is more appropriate in a
 longitudinal context. As seen in Fig.~\ref{fig:story} (last column,
 30 views) our method uses the prior scans }\emph{\DIFadd{uniformly}} \DIFadd{by
 creating an eigenspace representation
 (Section~\ref{sec:method_uniform_prior}).
}

 \DIFaddend However, with the use of such an object prior, a new challenge
 emerges\DIFdelbegin \DIFdel{:  the prior information }\DIFdelend \DIFaddbegin \DIFadd{. The prior set S }\DIFaddend may potentially overwhelm the \DIFdelbegin \DIFdel{essential new detailsthat appear in the
 test. It is in this context that the work in this paper fits in: the design of }\DIFdelend \DIFaddbegin \DIFadd{necessary
 details, or since several prior scans are available, finding the
 right prior may be an issue. We therefore seek }\DIFaddend an algorithm that
 estimates the location and magnitude of new changes in the (unknown)
 test\DIFdelbegin \DIFdel{object. This }\DIFdelend \DIFaddbegin \DIFadd{. As we show in Section~\ref{sec:method_spatially_varying_prior},
 this }\DIFaddend eventually prevents the prior from adversely affecting the
 reconstruction of new regions in the test. We refer to this \DIFaddbegin \DIFadd{method }\DIFaddend as
 a \textit{spatially-varying} \DIFdelbegin \DIFdel{uniform }\DIFdelend \DIFaddbegin \DIFadd{global }\DIFaddend prior-based reconstruction
 routine\DIFaddbegin \DIFadd{, that still uses all the previous scans in the set $S$}\DIFaddend .
\DIFdelbegin \DIFdel{We present results on real and simulated }\DIFdelend \DIFaddbegin 


 \DIFadd{This paper is organized as follows. After discussing related work in
 ...
 corresponding results in real and synthetic }\DIFaddend 3D \DIFdelbegin \DIFdel{data for different longitudinal studies. These are studies wherein sequential CT scans of the same object are acquired to track time-evolving changes within its interior}\DIFdelend \DIFaddbegin \DIFadd{biological datasets in
 ...
 drawn from our work in Section~\ref{sec:conclusions}}\DIFaddend .


%DIF >  We present results on real and simulated 3D
%DIF >   data for different longitudinal studies.  These are studies wherein
%DIF >   sequential CT scans of the same object are acquired to track
%DIF >   time-evolving changes within its interior.
 \DIFaddbegin 

  \DIFaddend %although this optimizer removes most of the artefacts created due to sub-sampling, its reconstruction is blurred when the number of views is very limited, as shown in Fig.~\ref{fig:story2}. 
...
\DIFdelbegin \DIFdel{Most of the related work for }\DIFdelend \DIFaddbegin \label{sec:related}
\DIFadd{Few view projection has been explored in~\mbox{%DIFAUXCMD
\cite{Essam2015,Polak2017}
}%DIFAUXCMD
and~\mbox{%DIFAUXCMD
\cite{Li2015}}%DIFAUXCMD
. However
}{\color{red} \DIFadd{add here!!  Why is this bad and how does it compare with
ours? }}

\DIFadd{Most }\DIFaddend object-prior based reconstruction has been in the areas of
...
stationary even within a single complete scan. In \DIFaddbegin \DIFadd{contrast, in }\DIFaddend 4D-CT,
\DIFaddbegin \DIFadd{a set of 3D CT scans over short intervals of time are taken as }\DIFaddend the
object undergoes changes between successive scans. In both these
cases, it may be possible to use very specific object-properties for
reconstruction. For example, in~\cite{Van2015}, the knowledge of \DIFaddbegin \DIFadd{the
}\DIFaddend attenuation coefficient of the fluid \DIFdelbegin \DIFdel{was }\DIFdelend \DIFaddbegin \DIFadd{is }\DIFaddend used as a prior for
reconstructing its flow through gravel. In~\cite{Koen2020}, the
changes across the sucessive scan volumes is assumed to be continuous,
thus enabling the use of optical flow to model the motion between
...
motion-correction. Such a correction enables acquisition from fewer
views than would have otherwise not been possible for reconstruction
with adequate fidelity. In~\cite{daniil2015}, all the scan volumes are
reconstructed together using spatio-temporal regularization.
\DIFdelbegin \DIFdel{Unlike these methods, if however, the object-prior were not available in the first place, then other techniques presented in~\mbox{%DIFAUXCMD
\cite{Essam2015,Polak2017} }%DIFAUXCMD
and~\mbox{%DIFAUXCMD
\cite{Li2015} }%DIFAUXCMD
could be used for the reconstruction of a single volume from few projection views. }\DIFdelend 


\DIFaddbegin \section{\DIFadd{Contributions}}
\label{sec:contributions}
\DIFadd{This paper focusses on few-views reconstruction with an emphasis on
longitudinal studies.  Fig.~\ref{fig:prior_overview} provides an
overview. }\DIFaddend In contrast to the \DIFaddbegin \DIFadd{object }\DIFaddend prior-based studies
mentioned above, we reconstruct the current test object without any
assumption of continuity of changes or some knowledge of the
attenuation coefficients of the structures. We \DIFaddbegin \DIFadd{do not make any
temporal assumption in terms of time intervals such as in 4D CT
scans. We }\DIFaddend use the current measurements from few-views and previous
\DIFdelbegin \DIFdel{high-quality }\DIFdelend scans of the same object. \DIFdelbegin \DIFdel{We utilize the uniform global eigenspace prior presented in ~\mbox{%DIFAUXCMD
\cite{my_dicta_paper} }%DIFAUXCMD
which assume that }\DIFdelend \DIFaddbegin \DIFadd{A key idea in our work is the starting point
-- }\DIFaddend the new test volume \DIFdelbegin \DIFdel{lies within }\DIFdelend \DIFaddbegin \DIFadd{is close to }\DIFaddend the space spanned by the
eigenvectors of the multiple representative previously scanned
objects.
\DIFdelbegin \DIFdel{We then }\DIFdelend \DIFaddbegin 

%DIF >  We utilize the uniform global eigenspace prior presented
%DIF >  in~\cite{my_dicta_paper} which assume 

\DIFadd{We also }\DIFaddend demonstrate how the global prior can impose an inflexible
constant weight (and hence \DIFdelbegin \DIFdel{a }\DIFdelend \DIFaddbegin \DIFadd{an unnecessary }\DIFaddend bias) when reconstructing
...
\centering
	\includegraphics[width=0.5\textwidth]{../images/prior_cmb.png}
        \caption{Overview of our work. The choice of the number of
          ...
          by the goal in the application under consideration.  \DIFaddbeginFL \DIFaddFL{(a) }\DIFaddendFL When
          our goal is relatively simple, such as tracking the location
          of new changes while simultaneously reducing sub-sampling
          artefacts, we propose \DIFdelbeginFL \DIFdelFL{(a) }\DIFdelendFL acquiring measurements from \DIFdelbeginFL \DIFdelFL{a }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{an
          extremely }\DIFaddendFL small number of views (`few-view' imaging) and
          using uniform prior based reconstruction. \DIFaddbeginFL \DIFaddFL{(b) }\DIFaddendFL When our goal
          becomes more ambitious, such as observing details of the new
          changes while simultaneously reducing sub-sampling
          artefacts, we propose \DIFdelbeginFL \DIFdelFL{(b) }\DIFdelendFL acquiring measurements from a
...
\DIFdelbegin \DIFdel{This paper is organized as follows: Section~\ref{sec:contributions} lays down }\DIFdelend \DIFaddbegin \DIFadd{In summary, }\DIFaddend the key contributions \DIFdelbegin \DIFdel{of this work. Section~\ref{sec:tmh} demonstrates the utility of both the uniform and spatially-varying methods on a longitudinal medical dataset. Section~\ref{sec:method_uniform_prior} describes the construction of the uniform eigenspace prior, followed by the corresponding results in real and synthetic }\DIFdelend \DIFaddbegin \DIFadd{are
} \begin{itemize} 
\item \DIFadd{We create and present new }\DIFaddend 3D biological datasets\DIFdelbegin \DIFdel{in Section~\ref{sec:results_uniform_prior}.Section~\ref{sec:method_spatially_varying_prior} describes how the uniform prior needs to be modified when accurate details of new changes are to be observed. A spatially-varying technique offers a solution, the results of which are presented }\DIFdelend \DIFaddbegin \DIFadd{, and as seen in
  Fig.~\ref{fig:story} demonstrate the use of object priors when very
  few views are used. More results appear }\DIFaddend in
  Section~\DIFdelbegin \DIFdel{\ref{sec:results_spatially_varying_prior}. Section\ref{sec:discussion} discusses tuning of the hyperparameters involved, and limitations of our method.  Finally, we conclude  with key inferences that can be drawn from our work in Section~\ref{sec:conclusions}.
  }\section{\DIFdel{Contributions}}
%DIFAUXCMD
\addtocounter{section}{-1}%DIFAUXCMD
%DIFDELCMD < %DIFDELCMD < \label{sec:contributions}%%%
%DIFDELCMD < %%%
\DIFdel{This paper focusses on few-views reconstruction for longitudinal studies, and has the following key contributions:
}%DIFDELCMD < \begin{itemize}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{\ref{sec:results_uniform_prior} and in the supplementary
  material.  Our results consist of 3D reconstructions from real
  cone-beam projections.
  }\DIFaddend \item The scenarios where a uniform object prior is sufficient for
    the application at hand, and the scenarios for which it will fail,
    are demonstrated. \DIFdelbegin %DIFDELCMD < \item %%%
\item%DIFAUXCMD
\DIFdel{A weights map
    is }\DIFdelend \DIFaddbegin \DIFadd{This leads to the idea of a weights map
    }\DIFaddend designed to depict the location and strength of new changes at
    ...
    of the templates. \DIFaddbegin \DIFadd{Results appear in
    Section~\ref{sec:results_spatially_varying_prior}.
  }\DIFaddend \item \DIFdelbegin \DIFdel{Finally, the methods are validated  on real and syntheric 3D tomographic data of biological datasets from various longitudinal studies , and
    quatitative accuracies are presented using the Structural Similarity Index Metric (SSIM).
 }\DIFdelend \DIFaddbegin \DIFadd{We show the efficacy of our results in a real-life medical
...
\item \DIFadd{To track the position of the needle in a relatively well-reconstructed background.%DIF > Initially, when our goal is to only track the location of the probe while simultaneously reducing sub-sampling artefacts, we advocate capturing measurements from a small number of views (`few-view' imaging) and using uniform prior based reconstruction.
}\item \DIFadd{To accurately observe the new changes amidst a relatively well reconstructed background after the needle touches the tumor.%DIF > Later, when the probe is proximal to or touches the tumor, our goal is to observe details of tumor ablation while simultaneously reducing sub-sampling artefacts. In this case, we propose to capture measurements from a slightly higher number of views (`moderate-view' imaging) to acquire more information, and use a spatially-varying prior-based reconstruction. The spatially-varying technique moderates the effect of the prior in the reconstruction of new changes in the object being scanned.
} \end{enumerate}  
\DIFaddend 
...
%DIFDELCMD < \item %%%
\item%DIFAUXCMD
\DIFdel{To track the position of the needle in a relatively well-reconstructed background.%DIF < Initially, when our goal is to only track the location of the probe while simultaneously reducing sub-sampling artefacts, we advocate capturing measurements from a small number of views (`few-view' imaging) and using uniform prior based reconstruction.
}%DIFDELCMD < \item %%%
\item%DIFAUXCMD
\DIFdel{To accurately observe the new changes amidst a relatively well reconstructed background after the needle touches the tumor.%DIF < Later, when the probe is proximal to or touches the tumor, our goal is to observe details of tumor ablation while simultaneously reducing sub-sampling artefacts. In this case, we propose to capture measurements from a slightly higher number of views (`moderate-view' imaging) to acquire more information, and use a spatially-varying prior-based reconstruction. The spatially-varying technique moderates the effect of the prior in the reconstruction of new changes in the object being scanned.
}
 \end{enumerate} %DIFAUXCMD
%DIFDELCMD < \end{enumerate} 
%DIFDELCMD < %%%
\DIFdelend Here, we demonstrate the combined use of l1-ls (CS) with the global prior, in two flavors: the vanilla (uniform) and spatially-varying global prior-based reconstruction. The choice of the number of measurement views and the type of reconstruction -- uniform or spatially-varying, is driven by the goal of the procedure. First, in order to track the needle, a very small number of views is sufficient because the needle has a very high attenuation coefficient when compared to that of the organs. We use the uniform global prior reconstruction here to reduce the artefacts due to sub-sampling. The uniform method is fast and sufficient to track the position of the needle.  Once the needle reaches the site of the tumor, we propose changing the imaging protocol to acquire measurements from a moderate number of views. This will enable us to get more information about the new changes. In addition, we then deploy the spatially-varying prior method in order to locate the regions of new changes and penalize any dominance of the prior in these regions.  Regardless of the imaging protocol we use (`few' or `moderate'), the number of views is smaller (atleast one-fifth) than the conventional number of views used in a standard hospital setting.
...
\end{equation}
 ...
 The proposed method has been validated on new\footnote {These and our code will be made available to the community.} scans of biological specimens in a longitudinal setting. These datasets in the form of \emph{raw cone-beam projection measurements} were acquired from a lab at the Australian National University (ANU). %We emphasize that in most of the literature on tomographic reconstruction, the results are shown on reconstruction from projections \emph{simulated} from 3D volumes. This is because most CT scanners do not reveal the raw projections, and instead output only the full reconstructed volumes. Moreover, the process of conversion from the projections to the full volumes is proprietary. \emph{Departing from this, we demonstrate reconstruction results from raw projection data.}
...
\caption{3D reconstruction of the okra from $10\%$ projection
  views (b) has strong streak artefacts, (c) blurred, (d) The prior enables better reconstruction.The reconstructed volumes can be viewed in ~\cite{supp_paper}.}
...
\textbf{full Volume} & 1 (ideal)                     & 0.87        &  0.92       & \textcolor{red}{0.95}                                                                                   \\ \hline
\end{tabular}
...
\hline
                   & \textbf{\begin{tabular}[c]{@{}c@{}}ground\\truth\end{tabular}} & \textbf{FDK} & \textbf{l1-ls} & \textbf{\begin{tabular}[c]{@{}c@{}}Uniform\\prior + l1-ls\end{tabular}} \\ \hline
\textbf{Full volume}   & 1 (ideal)                    & 0.82        & 0.71       & \textcolor{red}{0.92}                                                                                                                       \\ \hline
\end{tabular}
\label{table:sprouts_ssim}
\end{table}
%----------------------------------------------------------------------
\section{Method: Spatially-Varying prior-based reconstruction}
\label{sec:method_spatially_varying_prior}

Although the uniform global prior can be very useful in some circumstances, as was shown in Sec.~\ref{sec:tmh}, it poses a major limitation when we want accurate details of the new changes. While the uniform prior compensates very well for the possible artefacts due to sparse measurements, it dominates the regions with new changes masking the signal, as seen in Fig.~\ref{fig:RFA2_few_views}-d. Ideally, we will want to impose the prior only in the regions that are common between the test and object-prior.  Our spatially-varying prior based reconstruction overcomes this limitation by minimizing the following cost function:
\begin{equation}
%   \setlength{\belowdisplayskip}{0pt} \setlength{\belowdisplayshortskip}{0pt}
%\setlength{\abovedisplayskip}{-2pt} \setlength{\abovedisplayshortskip}{-2pt}
...
The proposed method has been validated on 2D and 3D synthetic and real tomographic data of biological and medical specimens. The results below demonstrate the advantage of the spatially-varying global prior method over the uniform method.
...
\begin{tabular}{|l|c|c|c|c|c|c|c|c|}
\hline
   & \textbf{FDK} & \textbf{l1-ls} & \textbf{\begin{tabular}[c]{@{}c@{}}Uniform\\prior + l1-ls\end{tabular}} & \textbf{\begin{tabular}[c]{@{}c@{}}Spatially-Varying\\prior + l1-ls\end{tabular}} \\\hline
%\textbf{\begin{tabular}[c]{@{}c@{}}Potato\\size:(150,150,100)\\45 views\end{tabular}}  & 5 sec. & 8 min.  & 54 min. & 53 min. \\ \hline
\textbf{\begin{tabular}[c]{@{}c@{}}Okra\\size:(338,338,123)\\45 views\end{tabular}} & 6 sec. & 8 min.  & 25 min. & 27 min. \\ \hline
\textbf{\begin{tabular}[c]{@{}c@{}}Sprouts\\size:(130,130,130)\\xx views\end{tabular}}  &  &   &  &  \\ \hline
\end{tabular}
...
\begin{tabular}{|l|c|c|c|c|c|}
\hline
                   & \textbf{\begin{tabular}[c]{@{}c@{}}ground\\truth\end{tabular}} & \textbf{FDK} & \textbf{l1-ls} & \textbf{\begin{tabular}[c]{@{}c@{}}Uniform\\prior\\+ l1-ls\end{tabular}} & \textbf{\begin{tabular}[c]{@{}c@{}}Spatially-Varying\\prior\\+ l1-ls\end{tabular}} \\ \hline
