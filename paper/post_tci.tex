...
In the recent past, deep learning based techniques~\cite{anirudh},\cite{kim2019extreme},\cite{bo2021},\cite{special} have also been widely used for limited view CT reconstruction. For example, in~\cite{anirudh}, a neural network is trained to predict the complete sinogram from an incomplete one.% This complete sinogram is then fed to analytical reconstruction algorithms to obtain the final reconstruction.
 ~\cite{bo2021} presented a cascade of neural networks that predict full-view reconstruction from limited view FBP reconstruction while simlutaneously ensuring consistency between the predicted sinugram and the acquired sinogram. \cite{special} trains an adverserial network for artifact removal and edge preservation in limited view reconstruction. In this work, the ability of these techniques to
uncover new features from one or many example training images is not being disputed. Instead, we focus is on a new discrete algorithm that builds structural maps that can and does point to new features. Having said that, our work is complementary to learning-based teachniques and can fit alongside a deep learning framework. For example, in~\cite{kim2019extreme}, an adversarial network is used to predict a reconstruction from extremely sparse measurements. This is used as an initialization in  a traditional iterative reconstruction algorithm to compute the final reconstruction. Given this pipeline, our method can be used to further inform the iterative algorithm of the available template information, as presented in Fig.~\ref{fig:Lit}.
