...
The three hyper-parameters, $\lambda_1$, $\lambda_2$ and $k$
need to be chosen carefully for optimal reconstruction. In our
experiments, $\lambda_1$ was tuned to maximize the
...
tuning. Thus, we compare our reconstruction with the best possible baseline TV reconstruction by choosing the $\lambda_1$ that gives the best SSIM for TV reconstruction. Further, this value was retained for our spatially-varying
prior-based reconstruction as well, that is, the value optimized for
the TV-only regularizer is maintained for the method proposed in this
paper.  In principle, one could have jointly optimized both
$\lambda_1$ and $\lambda_2$ to get even better results compared to the
one we have in Sections~\ref{sec:results_with_imaging_model},
and~\ref{sec:results_without_imaging_model}. 

The value of $\lambda_2$
largely depends on the amount of artefacts one aims to remove by the
use of object-prior at the cost of their dominance in the new
regions. This value is chosen omnisciently, i.e., with the chosen $\lambda_1$, we apply our method for a range of $\lambda_2$ values and select the one that gives the best SSIM. This  was found to lie between $(0,3)$ for
our datasets. In practice however, one may select $\lambda_2$ omnisciently only while imaging a particular class of object (say `sprouts') for the first time, and thereafter retain the same $\lambda_2$ while imaging other objects of the same class in future. 
