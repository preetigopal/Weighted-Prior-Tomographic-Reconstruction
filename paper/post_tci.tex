...
\begin{abstract}
  ...
  regions and then imposing object-prior in \textit{only} old regions which are similar to the prior.
...
\begin{figure}[!h]
    ...
    \begin{subfigure}[b]{0.24\linewidth}
        ...
        \caption{combined}
     \end{subfigure}
      \caption{The motivation for multiple methods is demonstrated with a slice of the data in Fig.~\ref{fig:potato_datasett}.  The test image in Fig.~\ref{fig:potato_dataset}(d) is reconstructed using only  $6$ paltry views.  The weights-maps 
        %(corresponding to the difference between pilot reconstruction
        of the test and its projection onto the eigenspace
        $\boldsymbol{V_\text{low}}$) constructed using different
        ...
        Eq.~\ref{eq:weightsEq} we get the final weights map (h). The corresponding reconstructions appear in Fig.~\ref{fig:reconstruction_diff_methods}. 
