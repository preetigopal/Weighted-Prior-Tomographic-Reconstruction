...
parameters for cone-beam imaging model are available to us.
Hence all reconstructions on these datasets were performed directly
from real projection measurements. For the other two datasets namely, Sprouts and
Liver, the raw measurements and the parameters of imaging
...
The test volume was reconstructed from a partial set of 18 real
cone-beam projections, i.e., $2\%$ of the $900$ projection views from
...
\caption{Reconstruction of potato from 6 projection views--(b) has strong streak artefacts with unclear structure of the potato, (c) largely blurred, and (c) is sharper with significantly reduced streak artefacts. 3D reconstructions from 18 cone-beam views can be seen in the supplementary material~\cite{supp_paper}. }
...
The test volume was reconstructed from a partial set of 30 real
cone-beam projections, i.e., $6.7\%$ of the $450$ projection views from which
...
\caption{Reconstruction of okra from 48 projection views
   (b) has streaky artefacts, (c) has blurred structures and (d) is sharper with significantly less streaky artefacts. 3D reconstructions from 30 cone-beam views can be seen in the supplementary material~\cite{supp_paper}.}
...
The test volume was reconstructed from partial set of 30 
cone-beam projections, i.e., $1.7\%$ of the $1800$ projection views
...
In this section, we discuss tuning of parameters and the computational complexity of our method.
...
The use of object-priors does introduce additional time and space complexities when compared to the use of a simpler prior such as TV-only prior.  For spatially-varying prior, most of the
computation time is used in the generation of low quality eigen-space
by reconstructing each of the templates using TV and FDK. If the set of projection views for every test is fixed a priori, the set of low quality reconstructions can be produced offline and stored, in order to save computational costs. If however, new sets of projection views for every test are to be allowed, the low quality reconstructions can still be performed efficiently using parallelization since these reconstructions are completely independent. Here below, we describe the
...
\end{itemize}
where $O(.)$ is the notation for `Big O' used in complexity analysis. In terms of space, the complexity is $O(T)$ in order to store the templates and their low quality reconstructions. We need to store a single weights map volume regardless of the number of templates available. 
