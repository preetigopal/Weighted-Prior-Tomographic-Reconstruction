...
\centering
	\includegraphics[width=0.45\textwidth]{../images/story/post_TCI/story.png}
        \caption{Illustration of the use of priors in reconstruction
          ...
          (SSIM=0.87) or no prior (SSIM=0.64), an improvement by $4\%$ and $27\%$ respectively (since the ideal SSIM=1).}
 \label{fig:story}
 \end{figure} 

  When the number of views is reduced significantly further (as is discussed in this work), a
 ...
 one ``right'' prior. In this way, this work is novel and compared to
...
brings with it a stronger challenge.
...
$\lambda_1$ and  $\lambda_2$ are tunable weights given to the
TV-prior and object-prior terms respectively. Their selection is discussed in
Section.~\ref{sec:tuning_parameters}. The unknowns
...
absolute error. For purposes of normalization, while reporting numbers
we rescale intensity values of both the ground truth and the
reconstructed results to lie between 0 to 1, so that the best possible
RMSE is 0, and the worst value is 1.  On the other hand, SSIM seeks to
consider perceptual changes, as well as correlation between adjacent
pixels. This can be useful in a clinical context.  Here, we therefore
assign a higher weightage to structure preservation, that is we assign
the following weights to structure, contrast, and intensity
respectively: $0.7,0.2,0.1$. Our results are optimized for SSIM, but
it can be verified that in all cases for the various datasets
considered, our results are favourable with either metric.

Here below, we first discuss tuning of parameters followed by the
...
$\lambda_1$, $\lambda_2$ and $k$ are the three hyper-parameters that
need to be chosen carefully for optimal reconstruction. In our
experiments, $\lambda_1$ was tuned in such a way as to maximize the
SSIM of the complete reconstructed test volume using the TV-only
regularizer.  In particular, the RoI was not considered.  Now, this
value was retained for our spatially-varying prior-based
reconstruction as well.  In principle, one could have jointly
optimized both $\lambda_1$ and $\lambda_2$ to get even better results
compared to the one we have in Sections~\ref{sec:results_with_imaging_model}, and~\ref{sec:results_without_imaging_model}.  The value of
$\lambda_2$ largely depends on the amount of artefacts one aims to
remove by using object-prior at the cost of their dominance in the new
regions. This value was chosen to lie between $0-3$ for our datasets
(having the fixed $\lambda_1$).

The hyper-parameter $k$ defines the sensitivity of the weights map to
...
Alternatively, in cases where one wishes to completely avoid the use
of this hyper-parameter, one can construct a binary weights-map using
a learning based method described in the supplementary
...
\begin{table}[!h]
  \centering
\caption{SSIM and RMSE of the reconstructions shown in
...
\begin{table}[!h]
  \centering
  \caption{SSIM (RMSE) within the RoI of reconstructed potato from various
    methods. 
...
\begin{table}[!h]
  \centering
  \caption{SSIM (RMSE) within the RoI of reconstructed okra from various
    methods. 
...
\begin{table}[!h]
  \centering
  \caption{SSIM (RMSE) within the RoI of 2D reconstruction of radio-frequency ablation data of liver from various methods.  
...
\begin{table}[!h]
  \centering
  \caption{SSIM (RMSE) within the RoI of reconstructed sprouts from various
    methods. 
%The SSIM of ideal reconstruction) is 1. The ideal RMSE is 0, and the worst RMSE possible is 1 here. 
...
\begin{table}[!h]
 \centering
  \caption{Computation times (in seconds) for various stages of
    reconstruction of a 2D slice of Liver on MATLAB 2019a on an  AMD
    2920X 12-Core Processor machine with 64GB RAM. Our code is
    available for inspection~\cite{codeRepo}.}
\begin{tabular}{|l|l|l|l|}
\hline
 \begin{tabular}[c]{@{}l@{}} FBP (Sufficient \\ views)\end{tabular}             & \begin{tabular}[c]{@{}l@{}} TV (Moderate \\ views)  \end{tabular}                      & \begin{tabular}[c]{@{}l@{}}Computation of \\ weights map \\ (Very few \\views)\end{tabular} & \begin{tabular}[c]{@{}l@{}}Alternate\\ Minimization \\ (Very few \\views)\end{tabular} \\ \hline
...
assigning low prior weights wherever necessary. This innovation becomes evident through the weights maps shown here in Figures~\ref{fig:weights_map_2Dpotato} and~\ref{fig:few_view_okra_2D_weights}. The probability of
