...
In this context, measurements in conventional data acquisition techniques are acquired by sampling the physical object uniformly above the Nyquist rate. In such a case when there are sufficient measurements, reconstruction using the conventional filtered backprojection (FBP) suffices, as seen in Fig.~\ref{fig:story1}. However, in the last decade, reconstruction from sub-Nyquist sampled data has been made possible due to methods such as the widely used Compressed Sensing (CS) technique~\cite{Donoho,introCS}, which assumes the data to exhibit properties such as sparsity of the underlying image under certain mathematical transforms such as the Discrete Cosine Transform (DCT) or wavelet transforms. Fig.~\ref{fig:story2} shows how reconstruction using CS routine is better than FBP for sub-Nyquist number of views. Here, among the many optimizers used in CS, we have used the l1-regularized least squares (l1-ls) form and observed that although this optimizer removes most of the artefacts created due to sub-sampling, its reconstruction is blurred when the number of views is very limited, as shown in Fig.~\ref{fig:story2}. Specifically, the reconstruction in Fig.~\ref{fig:story2-CS} and~\ref{fig:story3-CS} is obtained by minimizing the following cost using the solver in~\cite{l1ls}:
 \begin{equation}
...
\centering
    \begin{subfigure}[b]{0.23\linewidth}
        \includegraphics[width=\textwidth]{../images/story/testIm.png}
        \caption{Test}
        \label{fig:story1-test}
     \end{subfigure}
    \begin{subfigure}[b]{0.23\linewidth}
        \includegraphics[width=\textwidth]{../images/story/600_views/fbp.png}
        \caption{FBP, no prior}
        \label{fig:1000_views_fbp}
    \end{subfigure}
    %\begin{subfigure}[b]{0.23\linewidth}
     %   \includegraphics[width=\textwidth]{../images/story/600_views/CS_lambda0_0.00.png}
     %   \caption{Least squares}
     %\end{subfigure}
    \caption{Reconstruction from large number of views (600 views); (a) 2D test slice of size [260, 260] (b) FBP reconstruction (SSIM = 0.92).}% (c) Least squares reconstruction (SSIM = 0.99).}
\label{fig:story1}
\end{figure}
...
\centering
    %\begin{subfigure}[b]{0.23\linewidth}
     ...
     %\end{subfigure}
    \begin{subfigure}[b]{0.23\linewidth}
        \includegraphics[width=\textwidth]{../images/story/100_views/fbp.png}
        \caption{FBP, no prior}
        \label{fig:1000_views_fbp}
    \end{subfigure}
    \quad
    \begin{subfigure}[b]{0.23\linewidth}
        \includegraphics[width=\textwidth]{../images/story/100_views/CS_lambda0_2.00.png}
        \caption{sparsity prior}
        \label{fig:story2-CS}
     \end{subfigure}
    \caption{Reconstruction of Fig.~\ref{fig:story1-test} from sub-Nyquist number of views (100 views); (a) FBP reconstruction (SSIM = 0.83) (b) l1-ls reconstruction (SSIM = 0.90).}
\label{fig:story2}
\end{figure}
...
\begin{enumerate}
\item Initially, when our goal is to only track the location of the probe while simultaneously reducing sub-sampling artefacts, we advocate capturing measurements from a small number of views (`few-view' imaging) and using uniform prior based reconstruction. 
...
This paper is organized as follows: Section~\ref{sec:tmh} demonstrates the utility of both the uniform and spatially-varying methods on a longitudinal medical dataset. Section~\ref{sec:uniform_global_prior} describes the construction of the uniform global eigenspace prior. In addition, we recap the advantages of global prior over dictionary priors. Section~\ref{sec:spatially-varying_prior} describes how the uniform prior needs to be modified when accurate details of new changes are to be observed. A spatially-varying technique offers a solution. Results validated on both synthetic and real 3D biological datasets are shown. Finally, we conclude with key inferences that can be drawn from our work in Section~\ref{sec:conclusions}.
...
\label{sec:uniform_global_prior}
Having presented the application, we first review the algorithm~\cite{my_dicta_paper} for a global (uniform) prior-based reconstruction in Sec.~\ref{sec:uniform_global_prior}. This is followed by details of our technique in Sec.~\ref{sec:spatially-varying_prior}. Our method estimates the location and magnitude of new changes, and eventually prevents the prior from adversely affecting the reconstruction of new regions in the test. This, we refer to as a \textit{spatially-varying} global prior-based reconstruction routine.
...
\label{sec:spatially-varying_prior}
