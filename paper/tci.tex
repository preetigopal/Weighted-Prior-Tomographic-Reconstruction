...
\centering
 ...
 longitudinal context. Specifically, longitudinal studies here refers
 to the acquisition of sequential CT scans of the same subject to
 track time-evolving changes within the subject's interior. As seen in
 Fig.~\ref{fig:story} (last column, 30 views) our method uses the
 prior scans \emph{uniformly} by creating an eigenspace representation
 ...
 a \textit{spatially-varying} prior-based reconstruction
 routine, that still uses \textit{all} the previous scans in the set $S$.
...
(Fig.~\ref{fig:object-prior_test_sprouts}). Projections were generated from the given volume of size 130$\times$130$\times$130. In contrast to the scientific
experiment performed for the case of the okra where we
