...
reconstruction of test, few other approaches have either estimated the
...
changes across the sucessive scan volumes are assumed to be
...
time. The methods that use object-specific knowledge
...
which may be infeasible to acquire.

Other approaches variously use robust principal components
...
framework~\cite{Van2014}. However, all these
...
directly in the projection space. In such techniques, the object prior
...
similar objects across time. We use these priors to improve the
...
purely data driven manner. Very importantly, our technique clearly
differentiates between genuine structural changes and ``changes'' that
appear due to undersampling artefacts.  Besides this, since we use a
statistical model with multiple object priors, we avoid the problem of
selecting an appropriate single object prior
